...
/* ── 점수 규칙 (해저탐험 정식 룰) ───────────────────────────────────────────
 * 3라운드 동안 잠수함의 공기를 나눠 쓰며 보물을 건진다.
 * 잠수함까지 살아 돌아와야 그 라운드의 보물이 점수가 된다 — 빠지면 0점.
 * 3라운드 뒤 보물 점수 합이 가장 큰 사람이 우승.
 */
const ROUNDS = 3;

export type DeepSeaDraft = {
  current: number;
  /** 라운드(0-based) → memberId → 가져온 보물 점수 문자열 */
  entries: Record<string, string>[];
};

/** 스토어의 unknown 초안을 안전하게 복원한다. 모양이 어긋나면 새로 시작한다. */
export function coerceDeepSeaDraft(v: unknown): DeepSeaDraft {
  const fresh: DeepSeaDraft = { current: 0, entries: Array.from({ length: ROUNDS }, () => ({})) };
  if (typeof v !== 'object' || v === null) return fresh;
  const o = v as Partial<DeepSeaDraft>;
...
export function deepSeaTotals(draft: DeepSeaDraft, players: Member[]): Record<string, number> {
...
type Props = {
  ...
  players: Member[];
  draft: unknown;
  onDraftChange: (draft: DeepSeaDraft) => void;
...
/**
 * 해저탐험 점수표 — 라운드마다 잠수함에 가져온 보물 점수를 적으면 누적을 계산한다.
 * 초안은 스토어(plays store의 toolDraft)에 있어 시트를 닫아도 유지된다.
 */
export function DeepSeaSheet({ visible, players, draft: rawDraft, onDraftChange, onCommit, pending, onClose }: Props) {
  const c = useTheme();
  const insets = useSafeAreaInsets();
  const draft = useMemo(() => coerceDeepSeaDraft(rawDraft), [rawDraft]);

  const totals = useMemo(() => deepSeaTotals(draft, players), [draft, players]);
      ...
      <View style={[styles.root, { backgroundColor: c.background, paddingTop: insets.top }]}>
        <View style={[styles.header, { borderBottomColor: c.border }]}>
          <View style={styles.headerText}>
            <Text style={[styles.caption, { color: c.accent }]}>해저탐험 점수표</Text>
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <Text style={[styles.caption, { color: c.textSecondary }]}>
            잠수함까지 살아 돌아온 사람만 보물 칩을 뒤집어 합계를 적으세요. 빠졌으면 0.
          </Text>
